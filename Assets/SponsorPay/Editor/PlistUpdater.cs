...
namespace UnityEditor.SponsorPayEditor
{
	public class PlistUpdater
	{
		...
		{
			var name = HasKey(doc.DocumentElement, "SPNetworkName").NextSibling.InnerText;
			var curr = dict.FirstChild;
			while(curr != null)
			{
		...
		public static void UpdatePlist(string path, string xmlNodeString)
		{
			...
			<array>
				<dict>
					<key>SPNetworkName</key>
					<string>NetworkAbc</string>
					<key>SPNetworkParameters</key>
					<dict>
			...
			*/
			XmlNode networks = HasKey(dict, "SPNetworks");			
			
			if(networks == null) {
				networks = AddChildElement(doc, dict, "key", "SPNetworks");
